...
export default function Search() {
    ...
    const [showMore,setShowMore] = useState(false);
      ...
      const handleChange = (e) => { // On basis of ID we are checking
        ...
        if (e.target.id === 'sort_order') {
            // Here we are splitting as we have given value as regularPrice_asc, so below sort will contain here regularPrice and order as asc!
      ...
      const onShowMoreClick = async () => {
        ...
        const urlParams = new URLSearchParams(location.search);
        urlParams.set('startIndex', startIndex); // from this index we now want to fetch! 
        const searchQuery = urlParams.toString();
        const res = await fetch(`/api/listing/get?${searchQuery}`);
        const data = await res.json();
        if (data.length < 9) {
        ...
        setListings([...listings, ...data]); // here we just add new listings to previous listings!
      };
  return (
    <div className='flex flex-col md:flex-row'>
      <div className='p-7  border-b-2 md:border-r-2 md:min-h-screen' id = 'left'>
